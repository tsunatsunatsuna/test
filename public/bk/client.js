...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const openModalButton = document.getElementById("openActressSearchButton");
  ...
  openGenreModalButton.addEventListener("click", openModal);
  ...
  // 女優検索フォームが送信されたときにもモーダルを閉じる
  actressSearchForm.addEventListener("submit", (event) => {
    event.preventDefault(); // ページがリロードされるのを防ぐ
    // ここで女優検索を行う処理を追加する
    //   closeModal(); // 検索後にモーダルを閉じる
  });
  ...
  apiForm.addEventListener("submit", async (event) => {
    ...
    const checkboxes = document.querySelectorAll(
      '#articleOptions input[type="checkbox"]:checked'
    );
    const articleParams = {};
    checkboxes.forEach((checkbox, index) => {
      articleParams[`article[${index}]`] = checkbox.value;
    });
    ...
    articleIds.forEach((id, index) => {
      articleIdParams[`article_id[${index}]`] = id;
    });

    // paramsオブジェクトにチェックボックスの値を追加
    const submitParams = { ...params, ...articleParams, ...articleIdParams };
    ...
    // 不要なarticleキーを削除
    delete submitParams.article_id;

    console.log(params);
    console.log(articleIdParams);
  ...
  actressSearchForm.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch("/api/actress", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      apiResults = actresses;
      displayResultsJoyu(actresses);
    } catch (error) {
      resultsDiv.textContent = "APIリクエストに失敗しました。";
    }
  });
  ...
  // 関数：女優結果-------------------------------------------------------------------------------
  function displayResultsJoyu(actresses) {
...
function updateArticleId() {
  const selectedActresses = document.querySelectorAll(
    'input[name="selectedActresses"]:checked'
  );
  const articleIdInput = document.getElementById("article_id");

  const ids = Array.from(selectedActresses).map((actress) => actress.value);
  articleIdInput.value = ids.join(" ");
}

// // Form1のsubmitイベント時に選択された女優のIDを追加
// document.getElementById('apiForm').addEventListener('submit', addSelectedActressesToArticleId);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  populateServiceOptions(sites[0].service);
  populateFloorOptions(sites[0].service[0].floor);
});

document.addEventListener("DOMContentLoaded", () => {
  const actressModal = document.getElementById("actressModal");
  const closeActressModalButton = document.getElementById(
    "closeActressModalButton"
  );
  const actressSearchForm = document.getElementById("actressSearchForm");

  const genreModal = document.getElementById("genreModal");
  const closeGenreModalButton = document.getElementById(
    "closeGenreModalButton"
  );
  ...
  genreSearchForm.addEventListener("submit", async (event) => {
    ...
    const response = await fetch(
      `https://api.dmm.com/affiliate/v3/GenreSearch?api_id=${apiID}&affiliate_id=${affiliateID}&floor_id=${floorID}&hits=100&offset=1&output=json`
    );
    const data = await response.json();
    console.log(data);
    displayResultsGenre(data.result.genre);
  });
...
function setFloorIdToForm() {
  ...
  if (floorIdInput) {
    ...
    newInput.name = "floor_id";
    newInput.value = floorId;
    genreSearchForm.appendChild(newInput);
  }
}
